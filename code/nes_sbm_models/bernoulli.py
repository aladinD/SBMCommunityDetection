...
def estimate_eta(edges: torch.Tensor, assignments: torch.Tensor, k: int) -> torch.Tensor:
    ...
    num_members = torch.sum(torch.nn.functional.one_hot(assignments, k).float(), 1)
    ...
    num_self_edges = torch.unsqueeze(torch.eye(k, device=DEV), 0) * torch.unsqueeze(num_members, 1)
